...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        # 这边和train不一样，之前train是初始化参数和生成模型，这边是直接通过文件名找到模型
        ckpt = tf.train.get_checkpoint_state(model_dir)#通过函数找到模型文件名
        if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
            print('Reloading model parameters..')
            model.saver.restore(sess, ckpt.model_checkpoint_path) #恢复变量
        else:
